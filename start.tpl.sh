...
start() {
  ...
  cd "$FOLDER_NAME" || exit
  if [ -f ./ca.crt ] ; then
    CORE_PEER_TLS_ROOTCERT_FILE=./ca.crt CORE_PEER_TLS_ENABLED=true CORE_TLS_CLIENT_CERT_PATH=./client.crt CORE_TLS_CLIENT_KEY_PATH=./client.key CORE_PEER_ADDRESS=$PEER_ADDRESS CORE_CHAINCODE_ID_NAME=$CHAINCODE_ID_NAME:$CHAINCODE_VERSION CORE_CHAINCODE_LOGGING_LEVEL=debug $BINARY_NAME &
  else
